...
void main(uint GI : SV_GroupIndex, uint3 DTid : SV_DispatchThreadID)
{
	...
	Src1 += SrcMip.SampleLevel(BilinearClamp, UV1 + float2(Off.x, 0.0), SrcMipLevel);
	Src1 += SrcMip.SampleLevel(BilinearClamp, UV1 + float2(0.0, Off.y), SrcMipLevel);
	Src1 += SrcMip.SampleLevel(BilinearClamp, UV1 + float2(Off.x, Off.y), SrcMipLevel);
	Src1 *= 0.25;
#endif
