...
void Model::CreateWithAssimp(const ModelLoadSettings& settings)
{
	...
	for (uint64 i = 0; i < numMaterials; ++i)
	{
		...
		if (mat.GetTexture(aiTextureType_DIFFUSE, 0, &diffuseTexPath) == aiReturn_SUCCESS)
			material.TextureNames[uint64(MaterialTextureType::Albedo)] = GetFileName(AnsiToWString(diffuseTexPath.C_Str()).c_str());
		...
		if (mat.GetTexture(aiTextureType_NORMALS, 0, &normalMapPath) == aiReturn_SUCCESS
			...
			material.TextureNames[uint64(MaterialTextureType::Normal)] = GetFileName(AnsiToWString(normalMapPath.C_Str()).c_str());
		...
		if (SponzaRoughnessMaps[i] != nullptr)
			material.TextureNames[uint64(MaterialTextureType::Roughness)] = GetFileName(SponzaRoughnessMaps[i]);

		if (mat.GetTexture(aiTextureType_AMBIENT, 0, &metallicMapPath) == aiReturn_SUCCESS)
			material.TextureNames[uint64(MaterialTextureType::Metallic)] = GetFileName(AnsiToWString(metallicMapPath.C_Str()).c_str());
	}
...
void Model::GenerateTestScene()
{
	...
	meshes.Init(3);
	meshes[0].InitPlane(Float3(0.0f, 0.0f, 0.0f), Float2(100.0f), Quaternion(), 0, vertices.Data(), indices.Data());
