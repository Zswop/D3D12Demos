...
TempRenderTarget* PostProcessHelper::GetTempRenderTarget(uint64 width, uint64 height, DXGI_FORMAT format, bool useAsUAV)
{
	for (uint64 i = 0; i < tempRenderTargets.Count(); ++i)
	{
		...
		if (rt.Texture.Width == width && rt.Texture.Height == height && rt.Texture.Format == format
			&& useAsUAV == (rt.UAV.ptr != 0))
		{
			tempRT->InUse = true;
			return tempRT;
		}
	...
	rtInit.Name = L"PP Temp Render Target";
...
ID3D12PipelineState* PostProcessHelper::GetPSO(CompiledShaderPtr pixelShader, const RenderTexture* const* outputs, uint64 numOutputs)
{
	...
	const RenderTexture* const* outputs, uint64 numOutputs)
{
	...
	ID3D12PipelineState* pso = GetPSO(pixelShader, outputs, numOutputs);
