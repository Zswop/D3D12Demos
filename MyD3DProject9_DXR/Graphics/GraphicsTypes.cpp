...
uint64 Buffer::MultiUpdateData(const void* srcData[], uint64 srcSize[], uint64 dstOffset[], uint64 numUpdates)
{
	...
	for (uint64 i = 0; i < numUpdates; ++i)
	{
		...
		uploadContext.CmdList->CopyBufferRegion(Resource, currOffset + dstOffset[i], uploadContext.Resource,
			uploadContext.ResourceOffset + uploadOffset, srcSize[i]);
...
void StructuredBuffer::UpdateData(const void* srcData, uint64 srcNumElements, uint64 dstElements)
{
	GPUAddress = InternalBuffer.UpdateData(srcData, srcNumElements * Stride, dstElements * Stride);
