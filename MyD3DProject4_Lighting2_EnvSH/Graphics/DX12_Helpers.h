...
const uint32 NumUserDescriptorRanges = 1;
const uint32 NumStandardDescriptorRanges = 5 + NumUserDescriptorRanges;
...
D3D12_STATIC_SAMPLER_DESC GetStaticSamplerState(SamplerState samplerState, uint32 shaderRegister = 0, uint32 shaderSpace = 0,
	D3D12_SHADER_VISIBILITY visibility = D3D12_SHADER_VISIBILITY_PIXEL);
D3D12_STATIC_SAMPLER_DESC ConvertToStaticSampler(const D3D12_SAMPLER_DESC& samplerDesc, uint32 shaderRegister = 0,
	uint32 shaderSpace = 0, D3D12_SHADER_VISIBILITY visibility = D3D12_SHADER_VISIBILITY_PIXEL);
...
void BindTempDescriptorTable(ID3D12GraphicsCommandList* cmdList, const D3D12_CPU_DESCRIPTOR_HANDLE* handles, uint64 count,
	uint32 rootParameter, CmdListMode cmdListMode);
