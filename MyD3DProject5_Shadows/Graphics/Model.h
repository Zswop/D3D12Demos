...
enum class MaterialTextureType : uint64
{
	Albedo = 0,
	Normal,
	Roughness,
...
struct MeshMaterial
{
	...
	const Texture* Textures[uint64(MaterialTextureType::Count)];
	uint32 TextureIndices[uint64(MaterialTextureType::Count)];

	uint32 Texture(MaterialTextureType texType) const
	{
		Assert_(uint64(texType) < uint64(MaterialTextureType::Count));
...
// void MakeConeGeometry(uint64 divisions, MeshVertex* dstVertices, uint16* dstIndices);
