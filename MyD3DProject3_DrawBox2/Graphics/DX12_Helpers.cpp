...
D3D12_STATIC_SAMPLER_DESC GetStaticSamplerState(SamplerState samplerState, uint32 shaderRegister, 
	uint32 shaderSpace,	D3D12_SHADER_VISIBILITY visibility)
{
	Assert_(uint64(samplerState) < ArraySize_(SamplerStateDescs));
	return ConvertToStaticSampler(SamplerStateDescs[uint64(samplerState)], shaderRegister, shaderSpace, visibility);
}
...
D3D12_GPU_DESCRIPTOR_HANDLE TempDescriptorTable(const D3D12_CPU_DESCRIPTOR_HANDLE* handles, uint64 count)
{
	...
	uint32 destRange[1] = { uint32(count) };
	DX12::Device->CopyDescriptors(1, &tempAlloc.StartCPUHandle, destRange, uint32(count), handles, DescriptorCopyRanges, SRVDescriptorHeap.HeapType);
...
void BindTempConstantBuffer(ID3D12GraphicsCommandList* cmdList, const void* cbData, uint64 cbSize,
	uint32 rootParameter, CmdListMode cmdListMode)
{
