...
void Initialize_Upload()
{
	...
	const std::wstring shaderPath = L"Shaders\\DecodeTextureCS.hlsl";
...

	{
		...
		rootParameters[ConvertParams_SRVIndex].ParameterType = D3D12_ROOT_PARAMETER_TYPE_32BIT_CONSTANTS;
...
void ConvertAndReadbackTexture(const Texture& texture, DXGI_FORMAT outputFormat, ReadbackBuffer& readbackBuffer)
{
	...
	PIXMarker pix2Event(readbackCmdList, "Readback Texture");
	...
	convertCmdList->SetComputeRoot32BitConstant(ConvertParams_SRVIndex, inputSRV, 0);
	...
	if (texture.Cubemap)
		DX12::SRVDescriptorHeap.FreePersistent(inputSRV);
