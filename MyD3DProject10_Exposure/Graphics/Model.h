...
static const float SpotLightRange = 7.5;
static const float SpotShadowNearClip = 0.1000f;
static const float SpotLightIntensityFactor = 2500.0f;
...
struct MeshMaterial
{
	...
	bool32 AlphaTest() const
	{
		return Textures[uint32(MaterialTextures::Opacity)] != nullptr;
	}
};
...
struct ModelLoadSettings
{
	...
	bool GenerateTangentFrame = true;
};
...
public:
	...
	void CreateFromMeshData(const ModelLoadSettings& settings);
	...
	const Array<ModelSpotLight>& ModelSpotLights() const { return modelSpotLights; }
	const Array<PointLight>& PointLights() const { return pointLights; }
	const Array<SpotLight>& SpotLights() const { return spotLights; }
...
protected:
	...
	Array<ModelSpotLight> modelSpotLights;
