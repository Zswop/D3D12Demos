...
void LightCluster::Initialize(const Array<ModelSpotLight>* spotLights_)
{
...
void LightCluster::UpdateLights(const Camera& camera)
{
	...
	{
		const ModelSpotLight& spotLight = (*spotLights)[spotLightIdx];

		float angularAttenuationY = spotLight.AngularAttenuation.y;

		ClusterBounds bounds;
		bounds.Position = spotLight.Position;
		bounds.Orientation = spotLight.Orientation;
		bounds.Scale.x = bounds.Scale.y = std::tan(angularAttenuationY / 2.0f) * SpotLightRange * scaleCorrection;
		bounds.Scale.z = SpotLightRange;
		...
		intersectsCamera[spotLightIdx] = SphereConeIntersection(spotLight.Position, spotLight.Direction, SpotLightRange,
			angularAttenuationY, nearClipCenter, nearClipRadius);
	}
...
void LightCluster::RenderClusters(ID3D12GraphicsCommandList* cmdList, const Camera& camera)
{
...

	{
		...
		clusterConstants.NumLights = uint32(numLightsToRender);
