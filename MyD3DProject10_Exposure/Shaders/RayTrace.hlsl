...
StructuredBuffer< MaterialTextureIndices> MaterialBuffers[] : register(t0, space102);
...
static float2 SamplePoint(in uint pixelIdx, inout uint setIdx)
{
	...
	return SampleCMJ2D(RayTraceCB.CurrSampleIdx, RayTraceCB.SqrtNumSamples, RayTraceCB.SqrtNumSamples, permutation);
}
...
void RaygenShader()
{
	...
	if (payload.PathLength > RayTraceCB.MaxAnyHitPathLength)
...
static float3 PathTrace(in MeshVertex hitSurface, in MaterialTextureIndices material, in PrimaryPayload inPayload)
{
	...
	const bool AvoidCausticPaths = false;
	const bool enableDiffuse = metallic < 1.0f;
	const bool enableSpecular = !(AvoidCausticPaths && inPayload.IsDiffuse);

	const float3 diffuseAlbedo = lerp(baseColor, 0.0f, metallic);
	const float3 specularAlbedo = lerp(0.03f, baseColor, metallic);
	...
	// Apply sun light
	{
		...
		// Stop using the any-hit shader once we've hit the max path length, since it's *really* expensive
		if (inPayload.PathLength > RayTraceCB.MaxAnyHitPathLength)
			traceRayFlags = RAY_FLAG_FORCE_OPAQUE;
	...
	// Apply spot lights
	{
		...
		for (uint spotLightIdx = 0; spotLightIdx < RayTraceCB.NumLights; spotLightIdx++)
		{
			...
			{
				// Shoot a shadow ray to see if the light is occluded
				RayDesc ray;
	...
	else
	{
...

		{
			...
			//
			// See: https://blog.selfshadow.com/publications/turquin/ms_comp_final.pdf
			float2 DFG = GGXEnvironmentBRDFScaleBias(saturate(dot(normalTS, -incomingRayDirTS)), sqrtRoughness);
			throughput *= 1.0.xxx + specularAlbedo * (1.0f / DFG.x - 1.0f);
		}
	}
	...
	if (inPayload.PathLength + 1 < RayTraceCB.MaxPathLength)
	{
		PrimaryPayload payload;
		payload.Radiance = 0.0f;
	...
	else
	{
		...
		// Stop using the any-hit shader once we've hit the max path length, since it's *really* expensive
		if (inPayload.PathLength + 1 > RayTraceCB.MaxAnyHitPathLength)
			traceRayFlags = RAY_FLAG_FORCE_OPAQUE;
...
MaterialTextureIndices GetGeometryMaterial(in uint geometryIdx)
{
	StructuredBuffer<GeometryInfo> geoInfoBuffer = GeometryInfoBuffers[RayTraceCB.GeometryInfoBufferIdx];
	const GeometryInfo geoInfo = geoInfoBuffer[geometryIdx];

	StructuredBuffer<MaterialTextureIndices> materialBuffer = MaterialBuffers[RayTraceCB.MaterialBufferIdx];
...
void ClosestHitShader(inout PrimaryPayload payload, in HitAttributes attr)
{
	...
	const MaterialTextureIndices material = GetGeometryMaterial(GeometryIndex());
...
void AnyHitShader(inout PrimaryPayload payload, in HitAttributes attr)
{
	...
	float opacity = opacityMap.SampleLevel(MeshSampler, hitSurface.UV, 0.0f).x;
	if (opacity < 0.35f)
		IgnoreHit();
}
