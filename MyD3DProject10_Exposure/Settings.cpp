...
namespace Framework
{
    ...
    void SettingsContainer::Update(uint32 displayWidth, uint32 displayHeight, const Float4x4& viewMatrix)
    {
        if (opened == false)
        {
            ...
            if (ImGui::Begin("settings_button", nullptr, ImVec2(75.0f, 25.0f), 0.0f, flags) == false)
            {
                ImGui::End();
                return;
            }
        ...
        for (uint64 groupIdx = 0; groupIdx < groups.Count(); ++groupIdx)
        {
            ...
            if (ImGui::CollapsingHeader(group.Name.c_str(), nullptr, true, group.Expanded) == false)
                continue;
