...
static const char* ProfileStrings[] =
{
		...
		"vs_6_0", "hs_6_0", "ds_6_0", "gs_6_0", "ps_6_0", "cs_6_0",
	#else
...
static Hash CompilerHash = MakeCompileHash();
...
static string GetExpandedShaderCode(const wchar* path, GrowableList<wstring>& filePaths)
{
	...
	while (true)
	{
		...
		string line = fileContents.substr(lineStart, lineEnd);
		if (line.find("#inlcude") == 0)
		{
			...
			else
			{
				...
				size_t endQuote = line.find('>', startQuote + 1);
				string includePath = line.substr(startQuote + 1, endQuote - startQuote - 1);
				fullIncludePath = FrameworkDir() + L"Shaders\\" + AnsiToWString(includePath.c_str());
			}
...
static string MakeDefinesString(const D3D_SHADER_MACRO* defines)
{
	...
	while (defines && defines->Name != nullptr && defines->Definition != nullptr)
	{
...
class FrameworkInclude : public ID3DInclude
{
	HRESULT Open(D3D_INCLUDE_TYPE IncludeType, LPCSTR pFileName, LPCVOID pParentData, LPCVOID* ppData, UINT* pBytes) override
	{
		...
		if (IncludeType == D3D_INCLUDE_LOCAL)
			...
			filePath = FrameworkDir() + L"Shaders\\" + AnsiToWString(pFileName);
		else
			return E_FAIL;
	...
	const char* profileString, ID3DBlob** compiledShader, ID3DBlob** errorMessages)
{
	...
	wstring frameworkShaderDir = FrameworkDir() + L"Shaders";
	...
	DXCall(compiler->Compile(sourceCode, path, AnsiToWString(functionName).c_str(),
...
static void CompileShader(CompiledShader* shader)
{
	...
	shader->ByteCode = CompileShader(shader->FilePath.c_str(), shader->FunctionName.c_str(),
		shader->Type, defines, shader->ForceOptimization, filePaths);
...
CompiledShaderPtr CompileFromFile(const wchar* path, const char* functionName,
	ShaderType type, const CompileOptions& compileOpts, bool forceOptimization)
{
	CompiledShader* compiledShader = new CompiledShader(path, functionName, compileOpts, forceOptimization, type);
...
{
	return CompileFromFile(path, functionName, ShaderType::Vertex, compileOptions, forceOptimization);
}
...
PixelShaderPtr CompilePSFromFile(const wchar* path,
	...
	bool forceOptimization)
{
	return CompileFromFile(path, functionName, ShaderType::Pixel, compileOptions, forceOptimization);
}
...
GeometryShaderPtr CompileGSFromFile(const wchar* path,
	...
	bool forceOptimization)
{
	return CompileFromFile(path, functionName, ShaderType::Geometry, compileOptions, forceOptimization);
}
...
HullShaderPtr CompileHSFromFile(const wchar* path,
	...
	bool forceOptimization)
{
	return CompileFromFile(path, functionName, ShaderType::Hull, compileOptions, forceOptimization);
}
...
DomainShaderPtr CompileDSFromFile(const wchar* path,
	...
	bool forceOptimization)
{
	return CompileFromFile(path, functionName, ShaderType::Domain, compileOptions, forceOptimization);
}
...
ComputeShaderPtr CompileCSFromFile(const wchar* path,
	...
	bool forceOptimization)
{
	return CompileFromFile(path, functionName, ShaderType::Compute, compileOptions, forceOptimization);
}
...
bool UpdateShaders()
{
	...
	static uint64 currFileIdx = 0;
	currFileIdx = (currFileIdx + 1) % uint64(numShaderFiles);

	ShaderFile* file = ShaderFiles[currFileIdx];
...
void CompileOptions::Add(const std::string& name, uint32 value)
{
	...
	nameOffset[numDefines] = bufferIdx;
...
void CompileOptions::MakeDefines(D3D_SHADER_MACRO defines[MaxDefines + 1]) const
{
	...
	{
		defines[i].Name = buffer + nameOffset[i];
